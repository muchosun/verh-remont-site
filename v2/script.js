...
const steps = [
  ...
  { title: "Площадь и тариф", hint: "Расчет обновляется сразу при изменении площади или тарифа.", type: "area" },
  {
    title: "Цель ремонта",
  ...
  },
  {
    ...
    options: [
      ...
      ["Консультация по тарифам", "Понять подходящий уровень"],
    ],
  },
      ...
      <div class="tariff-options">
        ${Object.entries(tariffs).map(([key, item]) => `
          <button class="tariff-option ${state.tariff === key ? "is-active" : ""}" type="button" data-tariff="${key}">
            ...
            <span>${key === "standard" ? "Практичный ремонт" : key === "comfort" ? "Основной уровень" : "Индивидуально"}</span>
...
function renderFinal() {
  body.innerHTML = `
    <div class="success-box">
      ...
      Перейдите к форме: расчет, выбранный тариф и сценарий уже подготовлены.
    </div>
  `;
}
...
leadForm.addEventListener("submit", (event) => {
  ...
  leadSummary.innerHTML += `
    <div class="success-box">
      ...
      Связь: ${messenger}. В финальной версии эти данные отправятся в Telegram или CRM.
    </div>
  `;
});
