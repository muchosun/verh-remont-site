...
- Под каждой заявкой добавляется кнопка `Позвонить` с номером клиента. MAX открывает HTTPS-страницу сайта, а она запускает системный вызов; номер передаётся во фрагменте URL и не попадает в HTTP-запрос. Для упоминаний ответственных заполните `MAX_IGOR_USER_ID`, `MAX_IGOR_NAME`, `MAX_YURIY_USER_ID` и `MAX_YURIY_NAME`. Имя должно полностью совпадать с именем профиля MAX, а ID — быть числовым. Если переменные не заполнены, заявка всё равно будет доставлена, но без упоминаний.
