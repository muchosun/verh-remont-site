...
MAX больше не позволяет получить список чатов бота через `GET /chats`. Для разовой технической настройки есть скрипт `scripts/find-max-chat-id.js`: он выполняет только `GET /updates?types=bot_added` и **не отправляет** сообщений в MAX.

1. Убедитесь, что у бота нет активной webhook-подписки.
2. Удалите бота из целевого рабочего чата и добавьте обратно. Для получения событий из группового чата MAX рекомендует дать боту права администратора.
