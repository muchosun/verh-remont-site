...
const pinnedJourney = document.querySelector("[data-pinned-journey]");
...
function initProcessTimeline() {
  ...
  const steps = [...processTimeline.querySelectorAll("[data-process-step]")];
  if (!steps.length) return;

  const revealStep = (step) => {
    step.classList.add("is-visible");
    if (steps.every((item) => item.classList.contains("is-visible"))) {
...
function initPinnedJourney() {
  if (!pinnedJourney) return;

  const images = [...pinnedJourney.querySelectorAll("[data-pinned-image]")];
  const copies = [...pinnedJourney.querySelectorAll("[data-pinned-copy]")];
  const dots = [...pinnedJourney.querySelectorAll("[data-pinned-dot]")];
  const currentLabel = pinnedJourney.querySelector("[data-pinned-current]");
  ...
  const setActive = (index) => {
    ...
    copies.forEach((copy, copyIndex) => copy.classList.toggle("is-active", copyIndex === index));
  ...
  const update = () => {
    ...
    const rect = pinnedJourney.getBoundingClientRect();
    const scrollableDistance = Math.max(1, pinnedJourney.offsetHeight - window.innerHeight);
