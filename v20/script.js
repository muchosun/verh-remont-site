...
function initInfiniteRails() {
  ...
  document.querySelectorAll("[data-infinite-rail]").forEach((rail) => {
    ...
    beforeClones.slice().reverse().forEach((clone) => track.prepend(clone));
    afterClones.forEach((clone) => track.append(clone));

    let animationFrame = null;

    const getLoop = () => {
      const start = originalItems[0].offsetLeft;
      const cycle = afterClones[0].offsetLeft - start;
      return { start, cycle };
    };

    const centerOnOriginals = () => {
      const { start } = getLoop();
    ...
    const keepLooping = () => {
      animationFrame = null;
      const { start, cycle } = getLoop();
      if (!cycle) return;

      if (viewport.scrollLeft <= start - 8) {
        viewport.scrollLeft += cycle;
      } else if (viewport.scrollLeft >= start + cycle - 8) {
        viewport.scrollLeft -= cycle;
      }
    };
    ...
    const scheduleLoopCheck = () => {
      ...
      animationFrame = window.requestAnimationFrame(keepLooping);
    };

    const moveRail = (direction) => {
      viewport.scrollBy({
        left: direction * Math.max(220, viewport.clientWidth * 0.78),
    ...
    window.requestAnimationFrame(() => {
      centerOnOriginals();
      window.requestAnimationFrame(centerOnOriginals);
    });
  });
}
